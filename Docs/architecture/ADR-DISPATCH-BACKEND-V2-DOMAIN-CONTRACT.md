...
The modular monolith implements the unified Dispatch Backend V2 execution state machine, assignment offer lifecycle, and plan versioning. The backend persistence foundation, REST API v2 (`/api/v2`), readiness projection evaluator (`DispatchActivationReadinessQuery`), and transactional outbox messaging are fully implemented in the codebase. Existing v1 endpoints and web workflows operate with backward compatibility.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Operations Coordinator | All dispatches allowed by tenant/workspace policy | Create or edit a draft plan; create offers; propose the lead | Own offer only | Dispatch when readiness passes | No, unless separately granted an override capability | No | Cancel only where policy grants it; no unilateral reopen/archive | Read audit |
