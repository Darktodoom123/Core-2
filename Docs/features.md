...
| React Native field application | Partial | The active release targets Android phones running Android 11+. Versioned field API, Expo/React Native shell, SecureStore authentication, explicit pending-revocation logout recovery, actor-scoped durable SQLite outbox, reconnect replay, conflict recovery, and native device GPS integration pass package, unit, workflow, and component coverage. Sprint 1, Sprint 2, and Sprint 3 are complete for the field mobile application. iOS and tablets are outside the active release scope. |
