...
it('shows server-authoritative personnel eligibility, credentials, asset readiness, maintenance, and schedule conflicts', function () {
    ...
    $this->actingAs($dispatcher)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->has('personnel_candidates', 3)
            ->where('personnel_candidates.0.name', 'Available Driver')
            ->where('personnel_candidates.0.assignment_type', 'driver')
            ->where('personnel_candidates.0.availability.label', 'Available')
            ->where('personnel_candidates.0.credential.status', 'valid')
            ->where('personnel_candidates.0.eligible', true)
            ->where('personnel_candidates.1.name', 'Expired Operator')
            ->where('personnel_candidates.1.credential.status', 'expired')
            ->where('personnel_candidates.1.eligible', false)
            ->where('personnel_candidates.2.name', 'Unavailable Technician')
            ->where('personnel_candidates.2.availability.value', 'unavailable')
            ->where('personnel_candidates.2.eligible', false)
            ->has('asset_candidates', 3)
            ->where('asset_candidates.0.code', 'CR-5101')
            ->where('asset_candidates.0.blocking_maintenance_count', 1)
            ->where('asset_candidates.0.eligible', false)
            ->where('asset_candidates.1.code', 'EQ-5101')
            ->where('asset_candidates.1.schedule_conflicts.0.reference', 'CON-5100')
            ->where('asset_candidates.1.eligible', false)
            ->where('asset_candidates.2.code', 'TR-5101')
            ->where('asset_candidates.2.readiness.value', 'ready_for_service')
            ->where('asset_candidates.2.eligible', true)
        );
});
