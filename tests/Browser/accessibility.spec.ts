...
test.describe('UI-6 WCAG 2.2 AA Accessibility & Responsive Hardening', () => {
    ...
    test('mobile viewport 390px: sign-in form wraps cleanly and passes WCAG 2.2 AA', async ({
        page,
    }) => {
    ...
    test('authenticated operations workspace passes axe accessibility audit', async ({
        page,
    }) => {
    ...
    test('authenticated dispatch detail workspace passes axe accessibility audit', async ({
        page,
    }) => {
        const fixtures = browserFixtures();
        await signIn(page, fixtures.users.manager, fixtures.password);
