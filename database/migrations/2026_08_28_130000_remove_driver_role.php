...
return new class extends Migration
{
    ...
    private string $fallbackRoleName = 'crane_operator';
    ...
    public function up(): void
    {
        ...
        if ($fallbackRole !== null && Schema::hasTable($modelHasRolesTable)) {
            // Find all model assignments for driver role
            $assignments = DB::table($modelHasRolesTable)
                ->where('role_id', $role->id)
                ->get();

            foreach ($assignments as $assignment) {
                // Ensure model also has fallback crane_operator role
    ...
    public function down(): void
    {
        ...
        if (Schema::hasTable($roleTable)) {
            $exists = DB::table($roleTable)->where('name', $this->roleName)->exists();
            if (! $exists) {
