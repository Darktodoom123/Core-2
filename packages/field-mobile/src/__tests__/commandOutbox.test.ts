...
describe('CommandOutboxManager', () => {
    ...
    test('handles 409 Conflict without duplicate retries and sets actionable conflict state', async () => {
        const outbox = new CommandOutboxManager();
        const cmd = outbox.enqueueRespondAssignment(
            15,
            101,
            'accepted',
            undefined,
    ...
    test('clears all commands when the authenticated identity changes', () => {
        const outbox = new CommandOutboxManager();
        outbox.enqueueTransitionStatus(20, 'arrived', 1);
        outbox.enqueueShareLocation({
            dispatch_job_id: 20,
            latitude: 14.5995,
            longitude: 120.9842,
